...
def fetch_uuids(count=3):
    ...
    for i in range(count):
        try:
            response = httpx.get("https://jsonplaceholder.typicode.com/comments?postId=1", timeout=10.0)
            response.raise_for_status()
            data = response.json()
            for ele in data:
                uuids.append(ele["email"])
...
def pipeline_endpoint(request: PipelineRequest):
    ...
    if request.source != "JSONPlaceholder Comments":
        raise HTTPException(
            status_code=400, 
            detail="Source must be 'JSONPlaceholder Comments'"
        )
